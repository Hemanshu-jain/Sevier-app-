...
function BillingPage({ session }: { session: Session }) {
  ...
  return <>
    ...
    <section className="count-strip">
      <button type="button" onClick={() => scrollTo('recharge')}><strong>{rupees(summary.balancePaise)}</strong><span>Wallet balance</span></button>
      <button type="button" onClick={() => scrollTo('charges')}><strong>{rupees(summary.duePaise)}</strong><span>{summary.lockedCount} record{summary.lockedCount === 1 ? '' : 's'} waiting for recharge</span></button>
      <button type="button" onClick={() => scrollTo('imports')}><strong>{summary.month.count}</strong><span>Billed this month · {rupees(summary.month.amountPaise)}</span></button>
      <button type="button" onClick={() => scrollTo('charges')}><strong>{summary.allTime.count}</strong><span>Billed all time · {rupees(summary.allTime.amountPaise)}</span></button>
    </section>
    ...
    <article className="card settings-block" id="recharge">
