...
function MapsLink({ latitude, longitude }: { latitude?: number | null; longitude?: number | null }) {
  if (latitude == null || longitude == null) return null;
  return <a className="text-button maps-link" href={`https://www.google.com/maps?q=${latitude},${longitude}`} target="_blank" rel="noopener noreferrer"><MapPin size={14} /> Open field location in Maps</a>;
}
...
function App({ session, onLogout, onSessionUpdate }: { session: Session; onLogout: () => void; onSessionUpdate: (user: Session['user']) => void }) {
  ...
  async function assignCase(event: FormEvent<HTMLFormElement>) {
    ...
    if (!agentIds.length) { setActionError('Choose at least one agent.'); return; }
    try {
      setActionError('');
      await api.assignCase(session.token, selectedCase.id, agentIds, assignmentNote);
      await loadWorkspace();
      setDialog(null);
  ...
  async function revokeReleasePass() {
    if (!selectedCase) return;
    try {
      setActionError('');
      setActionNotice('');
      await api.revokeReleasePass(session.token, selectedCase.id, 'Revoked by finance');
      await loadWorkspace();
      setActionNotice('Release pass revoked. The verification page will now show it as invalid.');
    } catch (error) { setActionError(error instanceof Error ? error.message : 'Unable to revoke the release pass.'); }
  }
...
function AgentDialog({ session, onClose, onSubmit, onLinked }: { session: Session; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void; onLinked: () => void }) {
  ...
  return <Modal title="Add seizure agent" onClose={onClose}>
    ...
    {mode === 'explore' ? <div>
      ...
      <div className="directory-list">{results.length ? results.map((agent) => <div className="directory-row" key={agent.id}><div><strong>{agent.name} <RatingBadge rating={agent.rating} /></strong><small>{agent.city} · {agent.mobile} · {agent.createdVia === 'self' ? 'Self-registered' : 'Finance-added'} · {rateText(agent.rates, 'vehicle')}</small></div>{agent.linked ? <span className="linked-badge"><Check size={13} /> In roster</span> : <button className="secondary-button" type="button" disabled={busyId === agent.id} onClick={() => add(agent.id)}>Add</button>}</div>) : <p className="field-hint">No matching agents. Try another search, or create a new one.</p>}</div>
...
function AssignDialog({ caseItem, agentList, onClose, onSubmit }: { caseItem: RecoveryCase; agentList: Agent[]; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  const assignedIds = new Set((caseItem.assignedAgents ?? []).map((agent) => agent.id));
  const active = agentList.filter((agent) => agent.status === 'Active');
  return <Modal title="Assign seizure agents" onClose={onClose}><form onSubmit={onSubmit}><p className="modal-copy">Select one or more independent agents. Each receives the full case information and a notification. Unchecking an agent removes them from this case.</p><div className="case-reference"><strong>{caseItem.id}</strong><span>{caseItem.vehicle.registration} · {caseItem.borrower.name}</span></div><div className="assign-agent-list">{active.length ? active.map((agent) => <label key={agent.id} className="assign-agent-row"><input type="checkbox" name="agentIds" value={agent.id} defaultChecked={assignedIds.has(agent.id)} /><span><strong>{agent.name} <RatingBadge rating={agent.rating} /></strong><small>{agent.city} · {agent.activeCases} active · {rateText(agent.rates, 'vehicle')}</small></span></label>) : <p className="field-hint">No active agents in your roster. Add one from the directory first.</p>}</div><label className="field-label">Assignment note<textarea name="assignmentNote" maxLength={2000} defaultValue={caseItem.assignmentNote} placeholder="Optional instruction for the assigned agents" /></label><div className="modal-actions"><button className="secondary-button" type="button" onClick={onClose}>Cancel</button><button className="primary-button" type="submit">Save assignment</button></div></form></Modal>;
}
...
function ReleaseDialog({ caseItem, onClose, onConfirm }: { caseItem: RecoveryCase; onClose: () => void; onConfirm: () => void }) {
  return <Modal title="Create printable release pass" onClose={onClose}><form onSubmit={(event) => { event.preventDefault(); onConfirm(); }}><p className="modal-copy">Give this pass to the customer after payment is confirmed. The parking yard verifies it by scanning the QR code or checking the one-time verification token.</p><div className="release-pass-preview"><div><p>VEHICLE RELEASE PASS</p><strong>Generated securely</strong><span>{caseItem.vehicle.registration}</span></div><div className="qr-placeholder"><ShieldCheck size={18} /></div><small>Issued for {caseItem.borrower.name}<br />A unique pass ID and one-time verification code will be recorded.</small></div><label className="check-line"><input id="release-confirmation" required type="checkbox" /> I confirm payment clearance and customer details have been verified.</label><div className="modal-actions"><button className="secondary-button" type="button" onClick={onClose}>Cancel</button><button className="primary-button" type="submit"><FileCheck2 size={15} /> Generate pass</button></div></form></Modal>;
}
