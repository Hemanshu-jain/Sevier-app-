...
function App({ session, onLogout, onSessionUpdate }: { session: Session; onLogout: () => void; onSessionUpdate: (user: Session['user']) => void }) {
  ...
  return (
    <div className="app-shell">
      ...
      {selectedCase && <CaseDrawer caseItem={selectedCase} custody={custody.find((record) => record.id === selectedCase.custodyId)} evidence={caseEvidence} evidenceLoading={evidenceLoading} releasePass={releasePasses.find((pass) => pass.caseId === selectedCase.id)} session={session} onClose={() => { setSelectedCaseId(null); setDialog(null); }} onOpenDialog={setDialog} onPrint={printReleasePass} onRevokeAuthority={revokeAuthority} onRevokeRelease={revokeReleasePass} />}
