...
// These take an executor and DO NOT open their own transaction — the caller wraps
// them in tx() so the write is atomic with its audit/notification/receipt writes.
...
export async function persistReleasePass(executor, record) {
  const result = await query(executor,
